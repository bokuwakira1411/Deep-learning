...
class LSTM(nn.Module):
    def __init__(self, input_size, hidden_sizes, n_lstm_layers, n_hidden_layers, n_hidden_nodes, activation, epoch,
        ...
        self.lstm.append(nn.LSTM(input_size, hidden_sizes[0], batch_first=True))
        self.batch_norms_lstm.append(nn.BatchNorm1d(hidden_sizes[0]))
        ...
        for i in range(1, n_lstm_layers):
            self.lstm.append(nn.LSTM(hidden_sizes[i - 1], hidden_sizes[i], batch_first=True))
            #self.batch_norms_lstm.append(nn.BatchNorm1d(hidden_sizes[i]))

        self.hidden_layers = nn.ModuleList()
        self.hidden_layers.append(nn.Linear(hidden_sizes[-1], n_hidden_nodes[0]))
        for i in range(1, n_hidden_layers):
            self.hidden_layers.append(nn.Linear(n_hidden_nodes[i - 1], n_hidden_nodes[i]))
            self.batch_norms_fc.append(nn.BatchNorm1d(n_hidden_nodes[i-1]))
    ...
    def forward(self, x):
        ...
        #h = h.unsqueeze(1)
        #print(h.shape)
        #h = self.batch_norms_lstm[0](h[:,:,-1])  # Lấy hidden state cuối cùng
        ...
        for i in range(1, len(self.lstm)):
            h, _ = self.lstm[i](h)
            #h =h.unsqueeze(1)
            #h = self.batch_norms_lstm[i](h[:, -1, :])
            h = F.relu(h) if self.activation == 'relu' else torch.sigmoid(h)
            h = self.dropouts[i](h)

        for i in range(len(self.hidden_layers)):
            h = self.hidden_layers[i](h)
            if i < len(self.batch_norms_fc):
...
def main():
    ...
    model = LSTM(128, [256,512], 2, 3, [64,128,256], "relu", 25, [0,0,0.3,0.3,0.3], 0.001, 10, 10).to(device)
